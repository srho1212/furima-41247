    ...
    if (priceInput && addTaxDom && profitDom) { // 要素が存在するか確認
        priceInput.addEventListener("input", () => {
            ...
            if (!isNaN(inputValue)) { // 入力値が有効な数値であるかチェック
                ...
                // 利益と手数料の合計が入力値に一致するか確認し、調整が必要な場合
                if (fee + netProfit !== inputValue) {
                    const adjustedFee = inputValue - netProfit; // 利益を調整
                    addTaxDom.innerHTML = adjustedFee.toFixed();
                    profitDom.innerHTML = netProfit.toFixed();
                } else {
